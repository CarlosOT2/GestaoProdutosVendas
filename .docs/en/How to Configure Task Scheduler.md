...
2. Go to `Actions`, create a new Action by clicking New, click Browse, select the `delLocalRoot` file in the `back-end\data\local_credentials` directory, click Ok
3. Go to `Triggers`, create a new Trigger by clicking New, select the `On An Event` option in `Start the task`
